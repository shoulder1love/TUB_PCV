...
bool test_RANSAC(void)
{
    ...
    try{
        ...
        unsigned numInliers = countInliers(p_fst, p_snd, F, 10.0f);
        if (numInliers < 70) {
...
bool test_computeCameraPose(void){
    ...
    try{
        ...
        cv::Mat K = (cv::Mat_<float>(3, 3) << 2700, 0, 1500, 0, 2700, 1000, 0, 0, 1);

        cv::Mat H = computeCameraPose(K, p1, p2);
        correct = test_MatSize("Wrong dimensions!", H, 4, 4, 1) && correct;
        ...
        H.convertTo(H, CV_32FC1);
        correct = test_closeMat("", H_true, H) && correct;
        return correct;
    }
    catch(const std::exception &exc){
        cout <<  exc.what();
...
bool test_BundleAdjustment(void)
{
    ...
    try{
        ...
        for (unsigned i = 0; i < 5; i++) {
            auto p = K * C1Pose.dropLastRow() * tracks[i];
        ...
        for (unsigned i = 3; i < 8; i++) {
            auto p = K * C2Pose.dropLastRow() * tracks[i];
        ...
        for (unsigned i = 5; i < 10; i++) {
            auto p = K * C3Pose.dropLastRow() * tracks[i];
        ...
        for (unsigned i = 0; i < tracks.size(); i++) {
            ...
            state->m_tracks[i].location /= std::sqrt(innerProd(state->m_tracks[i].location, state->m_tracks[i].location));
        }
